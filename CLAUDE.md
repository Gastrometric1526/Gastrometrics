...
Sistema de gestión gastronómica (fichas técnicas, costeo, inventario, menús, órdenes de compra) para restaurantes en Centroamérica. Next.js 14 + React 18 + TypeScript + Tailwind + shadcn/ui. Backend real (Supabase) escrito para auth, planes y datos de negocio (ver `docs/52`) — pero las migraciones SQL más nuevas todavía no se han corrido contra el proyecto real, revisa `docs/52` antes de asumir que algo ya funciona en producción.
...
El backend real está en proceso de conexión, en fases (ver `docs/52`). **Fases 1-3 ya están escritas** (autenticación real, planes de suscripción reales con `account_plans`, e ingredientes/recetas/inventario/menús/órdenes de compra/negocios migrados a Supabase con una caché reactiva en memoria — ver `docs/52` para el patrón usado) — **pero las migraciones SQL nuevas (`0003`-`0005`) no se han corrido todavía contra el proyecto real**, así que nada de esto funciona en producción hasta que se corran (ver `docs/52`, sección "Migraciones pendientes"). Sin ellas, la app sigue arrancando y compilando bien, solo que cualquier lectura/escritura contra Supabase fallará. Lo que **sigue en `localStorage` a propósito** (Fase 4, menor prioridad/tráfico): `team.ts`, `feedback.ts`, `sales-imports.ts`. El checkout y el Portal de Cliente de Stripe **sí quedaron conectados de verdad** en `docs/49`, y el plan comprado ahora se aplica del lado del servidor (verificado contra Stripe, no confía en el navegador — ver `docs/52`). El buzón de `/contacto` también manda notificación por correo ahora (Resend, ver `docs/49`), condicionada a que existan las variables de entorno — sin ellas sigue funcionando igual que antes, solo sin el aviso.
