...
El backend real está en proceso de conexión, en fases (ver `docs/52`). **Fases 1-3 ya están hechas y verificadas en vivo** (autenticación real, planes de suscripción reales con `account_plans`, e ingredientes/recetas/inventario/menús/órdenes de compra/negocios migrados a Supabase con una caché reactiva en memoria — ver `docs/52` para el patrón usado): las migraciones `0003`-`0005` ya se corrieron contra el proyecto real, y se probó de punta a punta (registro → negocio → ingrediente → receta con cascada de costo, todo confirmado por API contra Supabase). De **Fase 4** ya se migró el buzón de `/contacto` (`0006_feedback.sql`, ver `docs/53`) — sigue en `localStorage` a propósito: `team.ts`, `sales-imports.ts`. El checkout y el Portal de Cliente de Stripe **sí quedaron conectados de verdad** en `docs/49`, y el plan comprado ahora se aplica del lado del servidor (verificado contra Stripe, no confía en el navegador — ver `docs/52`). El buzón de `/contacto` también manda notificación por correo ahora (Resend, ver `docs/49`) — ya con `RESEND_API_KEY`/`FEEDBACK_NOTIFY_TO` configuradas en local y en Vercel, y probado en vivo (envío real confirmado, sin errores en el servidor). `gastrometrics.org` (registrado en Cloudflare) ya está verificado en Resend y agregado como dominio real del proyecto en Vercel — el límite de "Resend solo manda a la casilla dueña de la cuenta" **ya no aplica**, confirmado con un envío real a un destinatario ajeno (ver `docs/53`). El correo de bienvenida (`app/api/auth/welcome-email/route.ts`) y la respuesta a feedback (`sendFeedbackReplyEmail`) ya mandan desde `notificaciones@gastrometrics.org`. Sigue pendiente (requiere que el dueño del proyecto pegue la API key de Resend en el dashboard de Supabase — Claude Code nunca escribe API keys en formularios): configurar SMTP en Supabase Authentication para poder editar las plantillas nativas de confirmación de registro/cambio de correo (instrucciones y plantillas listas en `docs/53`).

`NEXT_PUBLIC_SITE_URL` de producción apunta a `https://gastrometrics.org` (antes `gastrometrics-kappa.vercel.app`). El deploy a Vercel ya es automático en cada `git push origin main` — la conexión GitHub↔Vercel se depuró y quedó funcionando (ver `docs/53` para la causa real: la cuenta de Vercel estaba autenticada con un GitHub distinto al dueño del repo).

Recuperación de contraseña real (`/forgot-password` → `/reset-password`, vía `supabase.auth.resetPasswordForEmail`) y un panel en `/admin` ("Cuentas y planes") para buscar cualquier cuenta por correo y cambiarle el plan manualmente — el mecanismo para migrar testers a un plan pagado cuando el dueño decida, sin mover ningún dato (los negocios ya están ligados a la cuenta real, cambiar el plan no los toca). Ambos verificados en vivo, ver `docs/53`. De paso se corrigió un bug real de producción: el Site URL de Supabase Auth apuntaba a `localhost:3000` y la lista de Redirect URLs estaba vacía, lo que habría roto en producción cualquier link de correo de Auth (recuperación de contraseña incluida) — corregido en el dashboard de Supabase.

**Pendiente de que lo haga el dueño del proyecto** (Claude Code tiene prohibido escribir API keys en campos de formulario, incluso con autorización): configurar SMTP con Resend en Supabase Authentication → Emails para poder editar el asunto/cuerpo de los correos nativos de Supabase Auth (confirmación de registro, cambio de correo) — instrucciones exactas y las 3 plantillas listas para copiar/pegar en `docs/53`.
