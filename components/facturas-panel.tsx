...
import { Plus, Receipt, Search, Trash2, Pencil, Download, DollarSign, FileText, Info } from "lucide-react"
...
// Facturación a clientes — antes su propia página (`app/facturas/page.tsx`), movida
// acá como pestaña de /ventas (pedido explícito del dueño del proyecto: "creo que lo
// de invoice debería estar en lo de ventas, creo que eso tendría más sentido" — ambas
// son formas de registrar una venta real, así que comparten módulo). El gating por
// plan queda intacto (Home Cook+, `useFeatureAccess("invoices")`), pero ahora es
// interno a esta pestaña en vez de bloquear la página completa — así una cuenta con
// "invoices" pero sin "manual_sales"/"stats_finance" (Home Cook puro) puede seguir
// entrando a /ventas y usando Facturas, aunque las otras pestañas le queden bloqueadas.
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <p className="text-sm text-muted-foreground max-w-lg">{t("facturas_header_subtitle")}</p>
        <Button className="gap-2 shrink-0" onClick={openNewDialog}>
                ...
                {filteredInvoices.map((invoice) => (
                  <TableRow key={invoice.id}>
                    ...
                    <TableCell className="text-muted-foreground">
                      {/* BUG REAL CORREGIDO: "new Date(issueDate)" sobre un string plano
                      "YYYY-MM-DD" lo parsea como medianoche UTC — en cualquier huso horario
                      detrás de UTC (todo el continente americano) mostraba un día antes del
                      real. "T00:00:00" fuerza medianoche LOCAL, mismo patrón usado en el
                      resto del proyecto (ver lib/pdf/invoice-pdf-generator.ts). */}
